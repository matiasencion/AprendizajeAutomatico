"""Limpieza de partidos y construcción de atributos a partir del historial previo."""
...
def load_attributes(
    ...
    # Elo se calcula cronológicamente, conservando el criterio de actualización.
    elo_dict = {}
    elo_differences = []
    ...
    for idx, row in dataset.iterrows():
        ...
        # Se conserva la diferencia anterior al partido
...
def load_dataset(name_dataset, years_limit=1, matches_limit=5):
    df = load_base_dataset(name_dataset)
    return load_attributes(df, years_limit=years_limit, matches_limit=matches_limit)


def load_base_dataset(name_dataset):
    return clean_dataset(pd.read_csv(name_dataset))


def clean_dataset(df):
    """Limpia una tabla cruda sin depender de archivos externos."""
    df = df.copy()

    #Se seleccionan las columnas utilizadas por el clasificador
    ...
    #Se convierten las columnas a los tipos de datos correspondientes
    df["date"] = pd.to_datetime(df["date"], errors="raise")
    df["gh"] = pd.to_numeric(df["gh"], errors="raise").astype(int)
    df["ga"] = pd.to_numeric(df["ga"], errors="raise").astype(int)

    #Se ordenan los partidos por fecha y equipos, y se eliminan duplicados
    df = (
        df.drop_duplicates()
    ...
    #Se obtiene result: L para victoria local, V para victoria visitante y E para empate
    df["result"] = df.apply(
