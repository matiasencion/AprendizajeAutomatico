...
el analisis; los pruebo por separado.

Reglas del experimento del experimento:
- Solo se usa `train` (partidos anteriores a 2024-01-01). El test 2024-2025
  NO se toca en ningun momento.
- Se extiende la validacion cruzada de 2005-2023 (10 anios) a 2005-2023
  (19 anios), aplicando la recomendacion de usar mas anios de validacion
...
Se utilizan parámetros de referencia explícitos para este experimento independiente.
La entrega recalcula las ventanas con sus propios modelos seleccionados.
"""
...
def main():
    ...
    # Parametros de referencia; el notebook usa los ganadores de su propia corrida.
    ...
    def make_tree_pipeline():
        return create_model_pipeline(
            DecisionTreeClassifier(min_info_gain=0.00625, min_samples_leaf=1),
            **tree_best_params,
        )
    ...
    def report_one_se(rows, key="mean_f1_macro", se_key="se_f1_macro", value_label="valor"):
        ...
        print("Candidatos dentro de 1 SE del mejor (criterio descriptivo, no prueba de equivalencia):")
    ...
    # (years_limit=1, matches_limit=5) y se extiende la validacion a 2005-2023.
    # =========================================================
    print("=" * 70)
    ...
    t0 = time.time()
    dataset_default = load_dataset(dataset_path, years_limit=1, matches_limit=5)
    ...
    extended_validation_years = list(range(2005, 2024))  # 19 anios
    ...
    for years_limit in years_limit_values:
        ...
        if key not in datasets_cache:
            ...
            datasets_cache[key] = load_dataset(
                dataset_path, years_limit=years_limit, matches_limit=5
            )
    ...
    for matches_limit in matches_limit_values:
        ...
        if key not in datasets_cache:
            t0 = time.time()
            datasets_cache[key] = load_dataset(
                dataset_path, years_limit=1, matches_limit=matches_limit
            )
    ...
    out_path = task_dir / "experimentos" / "resultados" / "experimento_ventanas_resultados_actual.json"
